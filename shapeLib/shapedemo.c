...
{
  u_char row, col;
  Region bounds; //Rectangle around the shape
  ...
  for (row = bounds.topLeft.axes[1]; row < bounds.botRight.axes[1]; row++) {
    for (col = bounds.topLeft.axes[0]; col < bounds.botRight.axes[0]; col++) {
      Vec2 pixelPos = {col, row}; //Vector that points that position
      /*                */
      int color = abShapeCheck(shape, shapeCenter, &pixelPos) ? //give the shape if true draw fg
	fg_color : bg_color; //else draw bg color
